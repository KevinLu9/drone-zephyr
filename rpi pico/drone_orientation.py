...
class DroneOrientation:
    ...
    def __init__(self, i2c, orientation_lock, motor_lock):
        ...
        self.poll_frequency = (
            250  # Target Poll Frequency to calculate orientation from MPU6050 in Hz.
        )
    ...
    def _complementary_filter(
        self, gyro_data, accel_data, dt, alpha=0.98
    ):  # alpha = 0.98
    ...
    async def run(self):
        ...
        try:
            while not self.quit:
                ...
                self.pitch, self.roll, self.yaw = self._complementary_filter(
                    data["gyro"], data["accel"], dt
                )
                ...
                if (elapsed_time / 1000) < (1 / self.poll_frequency):
                    await asyncio.sleep(
                        (1 / self.poll_frequency) - (elapsed_time / 1000)
                    )
