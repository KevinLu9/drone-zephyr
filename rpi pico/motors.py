...
class Motors():
    def __init__(self, orientation_lock, drone_orientation, motor_lock):
        ...
        self.yaw_kd = 0
        self.poll_frequency = 250 # Target Poll Frequency to update motor thrust values in Hz.
        self.quit = False
    ...
    def set_throttle(self, throttle_fl, throttle_fr, throttle_bl, throttle_br):
        ...
        self.pwm_fl.duty_ns((int(throttle_fl * (self.MAX_THROTTLE - self.MIN_THROTTLE) + self.MIN_THROTTLE) * 1000))
        
        self.throttle_fr = throttle_fr
        self.pwm_fr.duty_ns(int((throttle_fr * (self.MAX_THROTTLE - self.MIN_THROTTLE) + self.MIN_THROTTLE) * 1000))
        
        self.throttle_bl = throttle_bl
        self.pwm_bl.duty_ns(int((throttle_bl * (self.MAX_THROTTLE - self.MIN_THROTTLE) + self.MIN_THROTTLE) * 1000))
        
        self.throttle_br = throttle_br
        self.pwm_br.duty_ns(int((throttle_br * (self.MAX_THROTTLE - self.MIN_THROTTLE) + self.MIN_THROTTLE) * 1000))
